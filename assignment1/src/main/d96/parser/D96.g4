...
statement: lhs | ifstatement | forstatement | returnstatement
	| continuestatement | breakstatement | invokemethod | vardecl | blockstatement;

// 6.2 Assignment statement
lhs: (scalavar | exp7 (LSB exp RSB)+) ASSIGN exp SEMI;
...
// 7.0 Expression block
invokemethod: (exp8 DOT ID | ID TWOCOLON DOLLAR_ID) LB explist? RB SEMI;
...
exp8: exp8 DOT ID (LB explist? RB)? | exp9;
exp9: ID TWOCOLON DOLLAR_ID (LB explist? RB)? | exp10;
exp10: NEW ID LB explist? RB | operands;

operands: value | SELFTYPE | NULLTYPE | ID | LB exp RB; 

// class definition 
classname: CLASSTYPE ID (COLON ID)? LP (method | attr | construct | destruct)* RP;
...
locals [lst = []]
@init {$attr::lst = [0,0,0]}
: (VARTYPE | CONSTTYPE) (DOLLAR_ID | ID) (COMMA {$attr::lst[0] += 1} (DOLLAR_ID | ID))*
...
arrayindex: ARRAYTYPE LB (exp (COMMA exp)* 
	| arrayindex (COMMA arrayindex)*)? RB;

// 3.7 Literals
value: INT | FLOAT | booln | STRING | arrayindex;
